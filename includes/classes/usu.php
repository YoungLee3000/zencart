...
class usu 
{
        ...
        foreach ($p as $index => $valuepair){
            ...
            switch ($p2[0]) {
                ...
                case 'id':    // EZ-Pages
                    ...
                    break;
				// Box News Module SEO
                case 'news_id':    // by weber
                    switch (true) {
                        case ($page == FILENAME_MORE_NEWS):
                            $url = $this->make_url($page, $this->get_news_box_title($p2[1]), $p2[0], $p2[1], USU_END, $separator);
                            break;
                        default:
                            $container[$p2[0]] = $p2[1];
                            break;
				...
				// Testimonials Module SEO
                case 'testimonials_id':    // by weber
                    switch (true) {
                        case ($page == FILENAME_TESTIMONIALS_MANAGER):
                            $url = $this->make_url($page, $this->get_testimonials_title($p2[1]), $p2[0], $p2[1], USU_END, $separator);
                            break;
                        default:
                            $container[$p2[0]] = $p2[1];
                            break;
    ...
    /**
     * Function to get the news title. Use database query in that order of precedent
     * @param integer $newsID
     * @return string news title
     */
    protected function get_news_box_title($newsID) 
    {
        global $db;

        if (defined('TABLE_BOX_NEWS_CONTENT')) {
             $sql = 'SELECT `news_title` AS `newsTitle` ' .
                    'FROM `' . TABLE_BOX_NEWS_CONTENT . '`' .
                    'WHERE `box_news_id`=\':newspage:\' ' .
                    'AND `languages_id` = \':language:\' ' .
                    'LIMIT 1';
             $sql = $db->bindVars($sql, ':language:', $this->languages_id, 'integer');
        }
        $sql = $db->bindVars($sql, ':newspage:', $newsID, 'integer');
        $result = $db->Execute($sql, false, true, 43200);
        $newsTitle = $this->filter($result->fields['newsTitle']);

        if (is_array($this->cache)) {
            $this->cache['BOXNEWS'][$newsID] = $newsTitle;
        }
    ...
    /**
     * Function to get the testimonial title. Use database query in that order of precedent
     * @param integer $tmID
     * @return string testimonials title testimonials_id
     */
    protected function get_testimonials_title($testimonialsID) 
    {
        global $db;

        if (defined('TABLE_TESTIMONIALS_MANAGER')) {
             $sql = 'SELECT `testimonials_title` AS `testimonialsTitle` ' .
                    'FROM `' . TABLE_TESTIMONIALS_MANAGER . '`' .
                    'WHERE `testimonials_id`=\':testimonialspage:\' ' .
                    'AND `language_id` = \':language:\' ' .
                    'LIMIT 1';
             $sql = $db->bindVars($sql, ':language:', $this->languages_id, 'integer');
        }
        $sql = $db->bindVars($sql, ':testimonialspage:', $testimonialsID, 'integer');
        $result = $db->Execute($sql, false, true, 43200);
        $testimonialsTitle = $this->filter($result->fields['testimonialsTitle']);

        if (is_array($this->cache)) {
            $this->cache['TESTIMONIALS'][$testimonialsID] = $testimonialsTitle;
        }
