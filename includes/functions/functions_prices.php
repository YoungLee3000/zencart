    ...
    if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
          ...
          $show_sale_discount = '<div class="product__label product__label--left product__label--sale productPriceDiscount"><span>'.PRODUCT_PRICE_SALE.'<br/>-' . $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span></div>';
		  
        } else {
          $show_sale_discount = '<div class="product__label product__label--left product__label--sale productPriceDiscount"><span>'.PRODUCT_PRICE_SALE.'<br/>-'. $currencies->display_price(($display_normal_price - $display_sale_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</span></div>';
        }
      } else {
        if (SHOW_SALE_DISCOUNT == 1) {
          $show_sale_discount = '<div class="product__label product__label--left product__label--sale productPriceDiscount"><span>'.PRODUCT_PRICE_SALE.'<br/>-'. number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span></div>';
        } else {
          $show_sale_discount = '<div class="product__label product__label--left product__label--sale productPriceDiscount"><span>'.PRODUCT_PRICE_SALE.'<br/>-' . $currencies->display_price(($display_normal_price - $display_special_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</span></div>';
        }
      }
    }

    if ($display_special_price) {
      $show_normal_price = '<span class="normalprice price-box__old">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </span>';
