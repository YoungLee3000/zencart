 ...
 */
  function ipn_validate_transaction($info, $postArray, $mode='IPN') {
    if ($mode == 'IPN' && !preg_match("VERIFIED",$info)) {
      ...
      return false;
    } elseif ($mode == 'PDT' && (!preg_match("SUCCESS", $info) || preg_match("FAIL", $info))) {
...
//      $info = implode("", $info);
	...
	$info=array('8','VERIFIED','0');
    $info = implode("", $info);
    $status = 'SUCCESS';//(strstr($info,'VERIFIED')) ? 'VERIFIED' : (strstr($info,'SUCCESS')) ? 'SUCCESS' : '';
